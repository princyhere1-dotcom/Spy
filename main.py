...
LOGS_CHAT_ID = -1002252848421  # This is the chat ID for the logs group
GAME_NOTIFICATION_CHAT_ID = -1002771343852
    ...
    if context.args and context.args[0].startswith("join_"):
        try:
            ...
            if len(game.players) >= 20:
                ...
                keyboard = [[InlineKeyboardButton("See the word", callback_data="check_word")]]
...
Use /begin to start the game"""
            ...
            keyboard = [[InlineKeyboardButton("JOIN GAME 🎮", url=deep_link_url)]]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
async def hints_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game.game_active:
        await update.message.reply_text("No active game to show hints for!")
        return

    # Build hints message with blockquote formatting
    hints_message = "🔍 Hints:\n\n"
    
    for idx, (player_id, player_name) in enumerate(game.players, 1):
        ...
        if player_hints:
            # Use blockquote formatting for each hint
    ...
    try:
        # Try to send with HTML parse mode for blockquote support
        await update.message.reply_text(hints_message, parse_mode='HTML')
    except Exception as e:
        logger.error(f"Failed to send hints with HTML formatting: {e}")
        # Fallback: send without formatting if HTML fails
        try:
            # Build simple text version
            simple_hints = "🔍 Hints:\n\n"
            for idx, (player_id, player_name) in enumerate(game.players, 1):
                if player_id in game.eliminated_players:
                    continue
            ...
            await update.message.reply_text(simple_hints)
        except Exception as e2:
            logger.error(f"Failed to send hints even without formatting: {e2}")
            await update.message.reply_text("Error displaying hints. Please try again.")
...
async def hint_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in game.eliminated_players:
        await update.message.reply_text("You have been eliminated and cannot give hints!")
        return
    ...
    if game.player_hints.get(user_id):
        await update.message.reply_text("💭 You already have given your hint")
        return
    ...
    game_keyboard = [
        [
            InlineKeyboardButton("Check the word", callback_data="check_word"),
            InlineKeyboardButton("Go to bot", url=bot_dm_link)
        ]
    ]
...
async def start_voting(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    for user_id, name in alive_players_after_kill:
        voting_keyboard.append([InlineKeyboardButton(name, callback_data=f"vote_{user_id}")])
...
async def send_voting_options_after_delay(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    try:
        ...
        for user_id, player_name in game.players:
            if user_id not in game.eliminated_players:
                keyboard.append([InlineKeyboardButton(player_name, callback_data=f"vote_{user_id}")])
...
def main():
    ...
    # Get bot token - hardcoded
    BOT_TOKEN = '7973919389:AAH9Bur_6nD-pDz7aIHzyDVKTV7xoak9Ae0'
    ...
    application.add_handler(CommandHandler("hints", hints_command))
